...
def _age_points(patient_df):
    ...
    """
    return max(
        0, min(4, (patient_df.loc[:, rename_helper("AGE")].max() - 40) // 10)
    )
...
def _aids_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("CHARLSON_AIDS")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Yes": 1,
            ...
            .max(),
            patient_df.loc[:, rename_helper("HIV")]
            .apply(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
                )
            )
            .max(),
        ]
...
def _cerebrovascular_accident_or_transient_ischemic_attack_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("CHARLSON_CVA_OR_TIA")]
        .apply(
            lambda value: {
                "0 = No": 0,
                "1 = Yes": 1,
...
def _chronic_obstructive_pulmonary_disease_points(patient_df):
    ...
    ret = pd.Series(
        [
            ...
            .max()
            .max(),
            patient_df.loc[:, rename_helper("CHARLSON_COPD")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Yes": 1,
...
def _congestive_heart_failure_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("CHARLSON_HEART_FAILURE")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Yes": 1,
            ...
            .max(),
            patient_df.loc[:, rename_helper("HEART_FAILURE")]
            .apply(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
                )
...
def _connective_tissue_disease_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("CHARLSON_CONNECTIVE_TISSUE_DISEASE")]
        .apply(
            lambda value: {
                "0 = No": 0,
                "1 = Yes": 1,
...
def _dementia_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("DEMENTIA")]
        .apply(
            lambda value: BOOLEANIZATION_MAP.get(str(value).lower(), np.nan)
        )
        .max()
    )
    if pd.isna(ret):
...
def _diabetes_mellitus_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("CHARLSON_DIABETES")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Uncomplicated": 1,
                    "2 = End-organ damage": 2,
                }.get(str(value), np.nan)
            )
            .max(),
            patient_df.loc[:, rename_helper(("DIABETES", ""))]
            .applymap(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
                )
...
def _leukemia_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("BLOOD_DISEASES")]
            .apply(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
                )
            ...
            .max(),
            patient_df.loc[:, rename_helper("CHARLSON_BLOOD_DISEASE")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Yes": 1,
            ...
            .max(),
            patient_df.loc[:, rename_helper("BLOOD_NEOPLASMS")]
            .apply(
                lambda value: {
                    "Any": 0,
...
def _liver_disease_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[
                :, rename_helper(("CHARLSON_LIVER_DISEASE", "LIVER_FAILURE"))
            ]
            .applymap(
                lambda value: {
                    "0 = No": 0,
    ...
    if all(
        (
            ...
            patient_df.loc[
                :,
                rename_helper(("HYPERTENSION", "")),
            ]
            .applymap(
                lambda value: BOOLEANIZATION_MAP.get(str(value).lower(), 0)
            )
            .max()
            .max()
            >= 1,  # plus hypertension
...
def _moderate_to_severe_chronic_kidney_disease_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("CHARLSON_KIDNEY_DISEASE")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "1 = Yes": 1,
            ...
            patient_df.loc[
                :,
                rename_helper(("CHRONIC_KIDNEY_DISEASE", "CKD")),
            ]
            .applymap(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
    ...
    if pd.isna(ret):
        raise FileNotFoundError()
    if ret == 0:
        max_creatinine = patient_df.loc[:, rename_helper("CREATININE")].max()
...
def _myocardial_infarction_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("CARDIAC_ISCHEMIA")]
        .apply(
            lambda value: BOOLEANIZATION_MAP.get(str(value).lower(), np.nan)
        )
        .max()
...
def _peptic_ulcer_disease_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("CHARLSON_PEPTIC_ULCER_DISEASE")]
        .apply(
            lambda value: {
                "0 = No": 0,
                "1 = Yes": 1,
...
def _peripheral_vascular_disease_points(patient_df):
    ...
    """
    ret = (
        patient_df.loc[:, rename_helper("CHARLSON_VASCULAR_DISEASE")]
        .apply(
            lambda value: {
                "0 = No": 0,
                "1 = Yes": 1,
...
def _solid_tumor_points(patient_df):
    ...
    ret = pd.Series(
        [
            patient_df.loc[:, rename_helper("CHARLSON_SOLID_TUMOR")]
            .apply(
                lambda value: {
                    "0 = No": 0,
                    "2 = Localized ": 2,
            ...
            .max(),
            patient_df.loc[:, rename_helper("SOLID_TUMOR")]
            .apply(
                lambda value: {
                    "Any": 0,
                    "Inactive": 2,
            ...
            .max(),
            patient_df.loc[:, rename_helper("NEOPLASMS")]
            .apply(
                lambda value: BOOLEANIZATION_MAP.get(
                    str(value).lower(), np.nan
                )
...
def compute_charlson_index(patient_df, valid_column_threshold=8):
    ...
    for function_returning_points in (
        ...
        _peripheral_vascular_disease_points,  # nobody has it ???
        _solid_tumor_points,
    ):
        try:
            cci += function_returning_points(patient_df)
        except FileNotFoundError:  # necessary columns were all empty :(
            _CHARLSON_COUNTER.update({function_returning_points.__name__: 0})
        else:
            valid_columns += 1
            _CHARLSON_COUNTER.update({function_returning_points.__name__: 1})
...
def max_charlson_col_length():
    return max(len(c) for c in _CHARLSON_COUNTER.keys())
